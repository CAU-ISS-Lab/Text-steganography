...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        x = th.rand(config.batch_size, 54, 64)
        if int(config.numm)==10:
            th.manual_seed(config.numm*13)
        #self.knoise= read_data(x,"cookies/noise/noise.txt",3)

        #self.prompt1= read_data(x,"cookies/noise/noise.txt",3)
        #self.prompt2= read_data(x,"cookies/noise/noise.txt",3)
    ...
    def p_mean_variance(
        ...
        #print(x.device)
        #print(self._scale_timesteps(t).device)
    ...
    def getsample(self,input_text,model,device,schedule_sampler):
        # for s2s (p: src, q (x_0_mean): tgt)
        print(input_text)
        length_tensor = th.tensor(54).repeat(input_text['tgt_input_ids'].size(0)).long()
        t, weights = schedule_sampler.sample(input_text['tgt_input_ids'].shape[0], device,seq_len=length_tensor.to(device),)
        q_input_ids = input_text['tgt_input_ids'].long().to(device)
    ...
    def p_skip_xy_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            text = th.randn(*shape, device=device)#self.text_k
            print("--------------", text.shape)
        ...
        for src_i, tgt_i in zip(src_indices, tgt_indices):
            ...
            with th.no_grad():
                ...
                self_cond = condition if self.config.infer_self_condition else None
                out = self.p_skip_sample(
                    model,
                    text,
                    src_t,
    ...
    def get_x_start(self, x_start_mean, std, prompt):
        '''
        ...
        noise = th.randn_like(x_start_mean)#prompt
    ...
    def training_losses_s2s(self, model, input_text, t, is_dev=False):
         ...
         #   line = line.strip().split(",")
          #  npp = []
           # for i in range(0, len(line) - 1):
            #    npp.append(int(line[i]))
            #if sum == t.shape[0]:
            #    break
            #for i in range(0, t.shape[1]):
             #       d = th.from_numpy(np.array(npp[0]))
              #      t[sum][i] = d
            #sum += 1
        #t=t.cuda()

        q_input_ids = input_text['tgt_input_ids'].long().to(t.device)
        #with open("./cookies/tgt_input_ids.txt", 'w', encoding='utf-8') as f:
         #   for i in range(0, q_input_ids.shape[0]):
          #      for j in range(0, q_input_ids.shape[1]):
           #         f.write(str(q_input_ids[i][j].cpu().detach().numpy()) + ',')
            #    f.write("\n")
        #f.close()

        x_start_mean = model.get_embeds(q_input_ids)  # because of DDP
        #with open("./cookies/x_start_mean_tgt.txt", 'w', encoding='utf-8') as f:
         #   for i in range(0, x_start_mean.shape[0]):
          #      for j in range(0, x_start_mean.shape[1]):
           #         for z in range(0,x_start_mean.shape[2]):
            #            f.write(str(x_start_mean[i][j][z].cpu().detach().numpy()) + ',')
             #       f.write("\n")
              #  f.write("\n")
        #f.close()
        ...
        noise = None
        if noise is None:
            noise=th.randn_like(x_start)#self.prompt1
        ...
        logits = model.get_logits(x_output)  # (bs, seq_len, vocab_size)

        sample_id_tensor = th.argmax(logits, dim=-1)
        #print(batch['tgt_input_ids_x'].shape, sample_id_tensor.shape)

        # print(cosine_distance(sample_id_tensor.detach().cpu().numpy(),batch['tgt_input_ids_x']))
        # input()
        each_sample_list2=[]
        tokenizer = AutoTokenizer.from_pretrained('bert-base-uncased')
        each_sample_list2.extend(
            tokenizer.batch_decode(sample_id_tensor, skip_special_tokens=True))

        output_path = os.path.join("generate_path", 'num')
        if 0 == 0:
            if not os.path.exists(output_path):
                os.makedirs(output_path)
        out_path = os.path.join(
            output_path, "rank" + str(0) + "_seed_" + "_y.txt")
        with open(out_path, 'w+', encoding='utf-8') as f:
            for sentence in tqdm(each_sample_list2):
                f.write(str(sentence) + '\n')
        f.close()

        return terms,x_output

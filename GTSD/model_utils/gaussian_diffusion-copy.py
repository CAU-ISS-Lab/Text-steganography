...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        self.text_k=th.randn(x.shape).cuda()
        self.text_k = self.text_k.cpu()
        ...
        for line in f:
            ...
            if sum == self.text_k.shape[1]:
                break
            for i in range(0, self.text_k.shape[0]):
                for k in range(0, self.text_k.shape[2]):
                    d = th.from_numpy(np.array(npp))
                    # d = th.FloatTensor(d)
                    # print(d)
                    self.text_k[i][sum][k] = d[k]
            sum += 1
        self.text_k = self.text_k.cuda()



        self.knoise= th.randn_like(x)
        self.knoise = self.knoise.cpu()
        f = open("noise.txt", encoding='utf-8')
        sum = 0
        for line in f:
            line = line.strip().split(",")
            npp = []
            ...
            for i in range(0, len(line) - 1):
                npp.append(float(line[i]))
            if sum == self.knoise.shape[1]:
                break
            for i in range(0, self.knoise.shape[0]):
                for k in range(0, self.knoise.shape[2]):
                    d = th.from_numpy(np.array(npp))
                    # d = th.FloatTensor(d)
                    # print(d)
                    self.knoise[i][sum][k] = d[k]
            sum += 1
        self.knoise = self.knoise.cuda()
    ...
    def p_skip_xy_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            #with open("111.txt", 'w',encoding='utf-8') as f:
             #   for i in range(0,text.shape[0]):
              #      for j in range(0,text.shape[1]):
               #         for l in range(0,text.shape[2]):
                #            f.write(str(text[i][j][l].cpu().numpy()) + ',')
                 #       f.write("\n")
                  #  f.write("\n")
            #f.close()
            #input()
            # self.getsample(input_text=data,model=model,device=device,schedule_sampler=schedule_sampler) #th.randn(*shape, device=device)
        ...
        for src_i, tgt_i in zip(src_indices, tgt_indices):
            ...
            with th.no_grad():
                ...
                self_cond = x_start if self.config.infer_self_condition else None
                #print('-----------', x_start, self.config.infer_self_condition)
                #print('-----------',self_cond,self.config.infer_self_condition)
                out = self.p_skip_sample(
                    model,
                    text,
                    src_t,
    ...
    def get_x_start(self, x_start_mean, std):
        ...
        for line in f:
            ...
            if sum == noise.shape[0]:
                break
            for i in range(0, noise.shape[1]):
                    d = th.from_numpy(np.array(npp[i]))
                    noise[sum][i] = d
            sum += 1
        noise=noise.cuda()

        with open("noise.txt", 'w', encoding='utf-8') as f:
            for i in range(0, noise.shape[0]):
                for j in range(0, noise.shape[1]):
                    f.write(str(noise[i][j].cpu().detach().numpy()) + ',')
                f.write("\n")
        f.close()
